...
drsyscall_os_init(void *drcontext)
{
    ...
    for (i = 0; i < count_syscall_ioctl_info; i++) {
        ...
        ASSERT(ok, "no dups");
        IF_DEBUG(ok =)
        hashtable_add(&name2num_table, (void *)info->name, (void *)&info->num);
        ASSERT(ok, "no dups");
    }
...
drsyscall_os_get_sysparam_location(cls_syscall_t *pt, uint argnum, drsys_arg_t *arg)
{
    ...
    if (IF_X86_ELSE(reg == DR_REG_EBP &&
                        reg_get_value(reg, arg->mc) != pt->sysarg[argnum],
                    false)) {
        /* must be vsyscall */
...
#define CHECK_DEF(ii, ptr, sz, id)                                                      \
    do {                                                                                \
        if (!report_memarg_type(ii, IOCTL_BUF_ARGNUM, SYSARG_READ, (byte *)ptr, sz, id, \
                                DRSYS_TYPE_STRUCT, NULL))                               \
            return;                                                                     \
    } while (0)
#define CHECK_ADDR(ii, ptr, sz, id)                                                      \
    do {                                                                                 \
        if (!report_memarg_type(ii, IOCTL_BUF_ARGNUM, SYSARG_WRITE, (byte *)ptr, sz, id, \
                                DRSYS_TYPE_STRUCT, NULL))                                \
            return;                                                                      \
    } while (0)
    ...
    case IPMICTL_SEND_COMMAND_SETTIME:
        ...
        if (safe_read((void *)arg, sizeof(struct ipmi_req_settime), &data.reqs)) {
            CHECK_DEF(ii, data.reqs.req.addr,
                      data.reqs.req.addr_len +
 ...
 */
static void
check_sockaddr(cls_syscall_t *pt, sysarg_iter_info_t *ii, byte *ptr, socklen_t socklen,
 ...
 */
static void
check_iov(cls_syscall_t *pt, sysarg_iter_info_t *ii, struct iovec *iov, size_t iov_len,
          size_t bytes_read, int ordinal, uint arg_flags, const char *id)
{
    uint i;
             ...
             size_t len, int ordinal, uint arg_flags)
{
    ...
    if (ii->arg->pre) {
        ...
        if (safe_read(&msg->msg_iov, sizeof(msg->msg_iov), &ptr1) &&
            safe_read(&msg->msg_iovlen, sizeof(msg->msg_iovlen), &len) && ptr1 != NULL) {
        ...
        if (safe_read(&msg->msg_control, sizeof(msg->msg_control), &ptr2) &&
            safe_read(&msg->msg_controllen, sizeof(msg->msg_controllen), &val_socklen)) {
            if (pt->first_iter) {
        ...
        if (safe_read(&msg->msg_iov, sizeof(msg->msg_iov), &iov) &&
            safe_read(&msg->msg_iovlen, sizeof(msg->msg_iovlen), &len) && iov != NULL) {
...
#    define SOCK_ARRAY_ARG 1
    ...
    case SYS_SOCKET:
        if (!report_memarg_type(ii, SOCK_ARRAY_ARG, SYSARG_READ, (app_pc)arg,
                                3 * sizeof(ptr_uint_t), "socket", DRSYS_TYPE_STRUCT,
                                NULL))
            return;
        break;
    ...
    case SYS_RECV:
        ...
        if (safe_read((void *)&arg[2], sizeof(arg[2]), &val_size) &&
            ...
            if (!report_memarg_type(ii, SOCK_ARRAY_ARG,
                                    request == SYS_SEND ? SYSARG_READ : SYSARG_WRITE,
                                    ptr1, val_size, id, DRSYS_TYPE_STRUCT, NULL))
                return;
        }
        break;
    case SYS_SENDTO:
    case SYS_RECVFROM:
        ...
        if (safe_read((void *)&arg[2], sizeof(arg[2]), &val_size) &&
            ...
            if (!report_memarg_type(
                    ii, SOCK_ARRAY_ARG,
                    request == SYS_SENDTO ? SYSARG_READ : SYSARG_WRITE, ptr1, val_size,
        ...
        if (((request == SYS_SENDTO &&
            ...
            if (request == SYS_SENDTO) {
                ...
                if (!report_memarg_type(ii, SOCK_ARRAY_ARG, SYSARG_WRITE, ptr1,
                                        val_socklen, "recvfrom addr", DRSYS_TYPE_STRUCT,
                                        NULL))
                    return;
    ...
    case SYS_RECVMSG:
        if (!report_memarg_type(
                ii, SOCK_ARRAY_ARG, SYSARG_READ, (app_pc)arg, 3 * sizeof(ptr_uint_t),
        ...
        if (safe_read((void *)&arg[1], sizeof(arg[1]), &ptr1)) {
            ...
            check_msghdr(drcontext, pt, ii, ptr1, sizeof(struct msghdr), SOCK_ARRAY_ARG,
                         (request == SYS_SENDMSG) ? SYSARG_READ : SYSARG_WRITE);
            if (ii->abort)
                return;
        }
        break;
...
handle_post_socketcall(void *drcontext, cls_syscall_t *pt, sysarg_iter_info_t *ii)
{
    ...
    case SYS_ACCEPT4:
        if (pt->sysarg[3] /*pre-addrlen*/ > 0 && pt->sysarg[2] /*sockaddr*/ != 0 &&
            ...
            safe_read(ptr2, sizeof(val_socklen), &val_socklen)) {
            check_sockaddr(pt, ii, SYSARG_AS_PTR(pt, 2, app_pc), val_socklen,
                           SOCK_ARRAY_ARG, SYSARG_WRITE, id);
            if (ii->abort)
                return;
        }
        break;
    ...
    case SYS_RECVFROM:
        ...
        if (pt->sysarg[3] /*pre-addrlen*/ > 0 && pt->sysarg[2] /*sockaddr*/ != 0 &&
            /* re-read to see size returned by kernel */
            safe_read((void *)&arg[5], sizeof(arg[5]), &ptr2) &&
            safe_read(ptr2, sizeof(val_socklen), &val_socklen) && val_socklen > 0) {
            check_sockaddr(pt, ii, SYSARG_AS_PTR(pt, 2, app_pc), val_socklen, 2,
                           SYSARG_WRITE, "recvfrom addr");
            if (ii->abort)
                return;
        }
        break;
    ...
    case SYS_RECVMSG: {
        if (pt->sysarg[2] != 0) { /* if 0, error on safe_read in pre */
            check_msghdr(drcontext, pt, ii, SYSARG_AS_PTR(pt, 2, byte *),
                         sizeof(struct msghdr), SOCK_ARRAY_ARG, SYSARG_WRITE);
            if (ii->abort)
                return;
        }
        break;
              ...
              int argnum_semid)
{
    ...
    case IPC_SET:
        if (ii->arg->pre) {
            if (!report_sysarg(ii, argnum_semid + 3 /*semun*/, SYSARG_READ))
                return;
            if (!report_memarg_type(ii, argnum_semid + 3, SYSARG_READ,
                                    (app_pc)arg_ptr->buf, sizeof(struct semid_ds),
                                    "semctl.IPC_SET", DRSYS_TYPE_STRUCT, NULL))
                return;
        }
        break;
    case IPC_STAT:
    case SEM_STAT:
        ...
        if (!report_memarg_type(ii, argnum_semid + 3, SYSARG_WRITE, (app_pc)arg_ptr->buf,
            ...
            return;
        break;
    case IPC_RMID: /* nothing further */ break;
    case IPC_INFO:
    case SEM_INFO:
        if (ii->arg->pre) {
            if (!report_sysarg(ii, argnum_semid + 3 /*semun*/, SYSARG_READ))
    ...
    case GETALL: {
        ...
        if (!report_memarg_type(ii, argnum_semid + 3, SYSARG_WRITE,
                                (app_pc)arg_ptr->array, semlen * sizeof(short),
                                "semctl.GETALL", DRSYS_TYPE_STRUCT, NULL))
            return;
        break;
    }
    case SETALL: {
        if (ii->arg->pre) {
            ...
            if (!report_memarg_type(ii, argnum_semid + 3, SYSARG_READ,
                                    (app_pc)arg_ptr->array, semlen * sizeof(short),
                                    "semctl.SETALL", DRSYS_TYPE_STRUCT, NULL))
                return;
        }
        break;
    }
    ...
    case IPC_SET: {
        if (ii->arg->pre) {
            ...
            if (!report_memarg_type(ii, argnum_buf, SYSARG_READ, (app_pc)buf,
                                    sizeof(*buf), "msgctl ipc_set", DRSYS_TYPE_STRUCT,
                                    NULL))
                return;
        }
        break;
    }
    case IPC_RMID: /* nothing further to do */ break;
    default:
...
handle_post_process_vm_readv(void *drcontext, sysarg_iter_info_t *ii)
{
    ...
    if (res > 0) {
        ...
        check_iov(ii->pt, ii, liov, (size_t)liovcnt, res, 1, SYSARG_WRITE, "local_iov");
    }
}
...
handle_post_process_vm_writev(void *drcontext, sysarg_iter_info_t *ii)
{
    ...
    if (res > 0 && pid == dr_get_process_id()) {
        ...
        check_iov(ii->pt, ii, riov, (size_t)riovcnt, res, 3, SYSARG_WRITE, "remote_iov");
    }
}
...
handle_pre_ipc(void *drcontext, cls_syscall_t *pt, sysarg_iter_info_t *ii)
{
    ...
    case SEMTIMEDOP:
        ...
        if (!report_sysarg(ii, 5, SYSARG_READ))
            return;
        if (!report_memarg_type(ii, 5, SYSARG_READ, (app_pc)arg5, sizeof(struct timespec),
                                "semtimedop", DRSYS_TYPE_STRUCT, NULL))
            return;
        /* fall-through */
    case SEMOP:
        ...
        if (!report_memarg_type(ii, 4, SYSARG_READ, (app_pc)ptr,
                                arg2 * sizeof(struct sembuf), "semop", DRSYS_TYPE_STRUCT,
                                NULL))
            return;
        break;
    case SEMGET: /* nothing */
...
handle_post_ipc(void *drcontext, cls_syscall_t *pt, sysarg_iter_info_t *ii)
{
    ...
    case MSGRCV: {
        if (result >= 0) {
            check_msgbuf(drcontext, pt, ii, (byte *)ptr, (size_t)result, 4, SYSARG_WRITE);
        }
        break;
    }
    case MSGCTL: {
...
handle_pre_select(void *drcontext, cls_syscall_t *pt, sysarg_iter_info_t *ii)
{
    ...
    if (ptr != NULL) {
        if (!report_memarg_type(ii, 1, SYSARG_READ, ptr, sz, "select readfds",
                                DRSYS_TYPE_STRUCT, NULL))
            return;
    }
    ptr = SYSARG_AS_PTR(pt, 2, app_pc);
    if (ptr != NULL) {
        if (!report_memarg_type(ii, 2, SYSARG_READ, ptr, sz, "select writefds",
                                DRSYS_TYPE_STRUCT, NULL))
            return;
    }
    ptr = SYSARG_AS_PTR(pt, 3, app_pc);
    if (ptr != NULL) {
        if (!report_memarg_type(ii, 3, SYSARG_READ, ptr, sz, "select exceptfds",
                                DRSYS_TYPE_STRUCT, NULL))
            return;
    }
    ...
    if (ii->arg->sysnum.number == SYS_pselect6) {
        ...
        if (ptr != NULL) {
            if (!report_memarg_type(ii, 5, SYSARG_READ, ptr, sizeof(kernel_sigset_t),
                                    "pselect sigmask", DRSYS_TYPE_STRUCT, NULL))
                return;
        }
    }
}
...
handle_pre_prctl(void *drcontext, cls_syscall_t *pt, sysarg_iter_info_t *ii)
{
    ...
    case PR_GET_NAME:
        if (!report_sysarg(ii, 1, SYSARG_READ))
            return;
        if (!report_memarg_type(
                ii, 1, (request == PR_GET_NAME) ? SYSARG_WRITE : SYSARG_READ,
                (app_pc)arg1, PRCTL_NAME_SZ, NULL, DRSYS_TYPE_CARRAY, NULL))
            return;
        break;
    default:
...
os_handle_pre_syscall(void *drcontext, cls_syscall_t *pt, sysarg_iter_info_t *ii)
{
    ...
    case SYS_clone: handle_clone(drcontext, pt, ii); break;
#ifdef X86
    case SYS__sysctl: {
        ...
        if (args != NULL) {
            ...
            if (!report_memarg_type(ii, 0, SYSARG_READ, (app_pc)args->name,
                                    args->nlen * sizeof(int), NULL, DRSYS_TYPE_STRUCT,
                                    NULL))
                return;
            if (args->newval != NULL) {
    ...
    case SYS_prctl: handle_pre_prctl(drcontext, pt, ii); break;
    ...
    case SYS_process_vm_writev: handle_pre_process_vm_readv_writev(drcontext, ii); break;
#ifdef X64
#    ifdef X86
    case SYS_arch_prctl: {
        ...
        if (code == ARCH_GET_FS || code == ARCH_SET_FS) {
            if (!report_memarg_type(ii, 1, SYSARG_WRITE, (app_pc)addr, sizeof(addr), NULL,
                                    DRSYS_TYPE_UNSIGNED_INT, NULL))
                return;
        } /* else, inlined */
        break;
...
os_handle_post_syscall(void *drcontext, cls_syscall_t *pt, sysarg_iter_info_t *ii)
{
        ...
        if (fds != NULL) {
            ...
            for (i = 0; i < nfds; i++) {
                ...
                if (!report_memarg_type(ii, 0, SYSARG_WRITE, (app_pc)&fds[i].revents,
                                        sizeof(fds[i].revents), NULL, DRSYS_TYPE_STRUCT,
                                        NULL))
                    return;
            }
        }
    ...
    case SYS_process_vm_writev: handle_post_process_vm_writev(drcontext, ii); break;
#ifdef X64
#    ifdef X86
    case SYS_arch_prctl: {
        int code = (int)pt->sysarg[0];
...
{
    cls_syscall_t *pt =
        (cls_syscall_t *)drmgr_get_cls_field(ii->arg->drcontext, cls_idx_drsys);
    check_sockaddr(pt, ii, start, (socklen_t)size, arg_info->param, arg_info->flags,
                   NULL);
    return true; /* check_sockaddr did all checking */
}

static bool
handle_msghdr_access(sysarg_iter_info_t *ii, const sysinfo_arg_t *arg_info, app_pc start,
                     uint size)
{
    cls_syscall_t *pt =
        (cls_syscall_t *)drmgr_get_cls_field(ii->arg->drcontext, cls_idx_drsys);
    check_msghdr(ii->arg->drcontext, pt, ii, start, (socklen_t)size, arg_info->param,
                 arg_info->flags);
    return true; /* check_msghdr checks whole struct */
}

static bool
handle_msgbuf_access(sysarg_iter_info_t *ii, const sysinfo_arg_t *arg_info, app_pc start,
                     uint size)
{
    cls_syscall_t *pt =
        (cls_syscall_t *)drmgr_get_cls_field(ii->arg->drcontext, cls_idx_drsys);
    check_msgbuf(ii->arg->drcontext, pt, ii, start, size, arg_info->param,
                 arg_info->flags);
                             ...
                             app_pc start, uint size)
{
    ...
    case SYSARG_TYPE_SOCKADDR: return handle_sockaddr_access(ii, arg_info, start, size);
    case SYSARG_TYPE_MSGHDR: return handle_msghdr_access(ii, arg_info, start, size);
    case SYSARG_TYPE_MSGBUF: return handle_msgbuf_access(ii, arg_info, start, size);
    case DRSYS_TYPE_CSTRARRAY: return handle_strarray_access(ii, arg_info, start, size);
    }
    return false;
}
